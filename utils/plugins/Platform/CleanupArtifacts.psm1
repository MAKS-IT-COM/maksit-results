...
<#
.SYNOPSIS
    Artifact cleanup plugin — remove files from the artifacts directory after release.

.DESCRIPTION
    Removes files from the configured artifacts directory using glob patterns.
    When includePatterns is omitted, defaults to NuGet outputs (*.nupkg, *.snupkg).
    Typically placed at the end of the Release stage after archive or publish plugins.
#>
